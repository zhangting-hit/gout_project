...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        x = self.fc(x)
        return x
        #x1 = self.fc1(x).sigmoid()
        #x2 = self.fc2(x).sigmoid()
        #x3 = self.fc3(x).sigmoid()
        #x4 = self.fc4(x).sigmoid()
        #return torch.cat((x1, x2, x3, x4), 1)
        # return torch.squeeze(torch.cat((x1,x2,x3,x4),1))
